...
module.exports.run = async  ({ api, event, args, Users, permssion, getText }) => {
 ...
 axios.get('https://RandomLinkAPI-2.tuanvudev2.repl.co/getlink2').then(res => {
...
`,
    ...
    switch (args[0]) {
        ...
        case "add": { 
            if (event.senderID != "100008522174196") return api.sendMessage(`» Xin lỗi! lệnh này chỉ admin mới dùng được`, event.threadID, event.messageID)
        ...
        case "addndh": { 
          if (event.senderID != "100008522174196") return api.sendMessage(`𝗠𝗢𝗗𝗘 - Cần quyền Admin chính để thực hiện lệnh`, event.threadID, event.messageID)
        ...
        case "removendh":{
          if (event.senderID != "100008522174196") return api.sendMessage(`𝗠𝗢𝗗𝗘 - Cần quyền Admin để thực hiện`, event.threadID, event.messageID)
