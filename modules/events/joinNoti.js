...
module.exports.run = async function({ api, event, Users }) {
	...
	if (event.logMessageData.addedParticipants.some(i => i.userFbId == api.getCurrentUserID())) {
		api.changeNickname(`『 ${global.config.PREFIX} 』 • ${(!global.config.BOTNAME) ? "♡ 𝑲𝒊𝒏𝒈 𝑩𝒐𝒃👑" : global.config.BOTNAME}`, threadID, api.getCurrentUserID());
		const fs = require("fs");
		return api.sendMessage("", event.threadID, () => api.sendMessage({body:`𝗞𝗘̂́𝗧 𝗡𝗢̂́𝗜 𝗧𝗛𝗔̀𝗡𝗛 𝗖𝗢̂𝗡𝗚\n━━━━━━━━━━━━\n→ Hế nhô, mị là Bot Cute của a chủ Văn Thịnh ❤\n→ Dùng lệnh menu để xem tất cả lệnh mị có\n→ Dùng lệnh -ad để xem thông tin của ADMIN\n→ Dùng callad để liên hệ nếu cần hỗ trợ nhé 🥰`, attachment: fs.createReadStream(__dirname + "/cache/joinbox/joinbox.mp4")} ,threadID));
	}
	else {
		try {
